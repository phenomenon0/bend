#!/usr/bin/env python3
# tests/power/deflate_mutants.py over std/: each mutant breaks
# std/deflate.bend the way a real codec goes wrong, and std/deflate_proof.bend
# must then fail to check. Every mutant is applied to a copy of std/ in a
# temporary directory (the checked-in files never change); its snippet must
# occur exactly once, and the mutated deflate.bend must still check on its
# own -- a mutant that does not type is a broken test, not a killed one. The
# fixture (tests/std/gzip.bend) is built for the C lane and run on the copy
# too, to show which mutants the oracle alone would also catch. --packed
...
MUTANTS = [
  ...
  ('a copy reads one byte too far back', 'deflate.bend', [(
    '+c = By.get(o, Nat.sub(By.len(o), dist))',
    '+c = By.get(o, Nat.sub(By.len(o), 1n+dist))')]),
  ('a distance reaching the first byte out is refused', 'deflate.bend', [(
    'copy.if(Nat.is_lt(By.len(out), dist),',
    'copy.if(Nat.is_le(By.len(out), dist),')]),
  ...
  ('a code is walked with its 1 bits taken as 0', 'deflate.bend', [(
    'walk(tab, U32.to_nat(By.get(tab, Nat.add(Nat.double(node), Lm.nat.bit(b, 0n)))), k, o, bp)',
    'walk(tab, U32.to_nat(By.get(tab, Nat.add(Nat.double(node), Lm.nat.bit(Bool.not(b), 0n)))), k, o, bp)')]),
  ...
  ('a copy is checked without its first byte', 'deflate.bend', [(
    'U32.is_eq(By.get(x, pos), By.get(x, Nat.sub(pos, dist))) && same(p, x, 1n+pos, dist)',
  ...
  ('the fast path walks a code with its 1 bits taken as 0', 'deflate.bend', [(
    'U32.to_nat(By.get(tab, Nat.add(Nat.double(node), Lm.nat.bit(b, 0n))))\n',
    'U32.to_nat(By.get(tab, Nat.add(Nat.double(node), Lm.nat.bit(Bool.not(b), 0n))))\n')]),
  ('a fast copy reads one byte too far back', 'deflate.bend', [(
    '  By.get(o, Nat.sub(By.len(o), dist))',
    '  By.get(o, Nat.sub(By.len(o), 1n+dist))')]),
...
def main():
  ...
  for name, target, edits in MUTANTS:
    ...
    try:
      ...
      shutil.copy(fixture, os.path.join(tmp, 'tests', 'std', 'gzip.bend'))
      if PACKED:
      ...
      src_path = os.path.join(tmp, 'std', target)
      ...
      code, out = run(['bun', bend, 'std/deflate_proof.bend'], tmp)
      exe = os.path.join(tmp, 'fx')
      code2, got = run(['bun', bend, 'tests/std/gzip.bend', '-o', exe], tmp)
      if code2 == 0:
        code2, got = run([exe], tmp)
