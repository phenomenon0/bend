...
`python3 net/mutants.py`: forty-one broken servers, streams and clients,
each refused; `python3 net/ws_mutants.py`: the WebSocket ones. Not
proven: the IO loops (they call the functions the laws are about), the
deadlines and limits (the handler's race is the runtime's IO.within,
bend2/effs/within.c), a stream route's head (checked by `check.py`).
...
asks too) and 101, SIGTERM's 1001.
...
whole, under max_body), WebSocket compression (permessage-deflate is
declined). Not possible: preempting a handler that computes without an
effect (the handler's time lets go only one that waits); a stream
route's handler has no handler time (it holds the socket), only its
reads' progress.
