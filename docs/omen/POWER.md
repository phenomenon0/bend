...
The same holds for `I64` and `U64`, which share the unboxed 64-bit slot:
`Bool.pick(Maybe<&2, I64>, False{}, Some{x}, None{})` faults when `x` is
`2^63 - 1` (json_value's `to_i64` met it on `-9223372036854775809`, and matches
instead).
