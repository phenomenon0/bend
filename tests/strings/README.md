...
`gaps.bend` (the Python-gap ops: predicates, edits, `String.format`) is
generated by `gaps_gen.py`, which computes every ASCII row's expectation with
CPython's own `str` methods; `run.sh` fails when the two disagree. Regenerate
with `python3 tests/strings/gaps_gen.py > tests/strings/gaps.bend`.
