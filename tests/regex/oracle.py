...
Pinned: refuses to run on anything but CPython 3.11.15 (re-execs into
$ORACLE_PYTHON when started by another python); the executable, versions and
sha256 of the executable, the `re` sources, this file, base.bend and the
corpus go to .tmp/regex/.
...
VERSION = (3, 11, 15)
PYTHON = os.environ.get("ORACLE_PYTHON", "")  # a CPython 3.11.15 executable
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
OUT = os.path.join(ROOT, ".tmp/regex")
CHUNK = 125
GROUP = 10
LIMIT = 4294967295
